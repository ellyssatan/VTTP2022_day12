...
public class WeatherController {
    ...
    @GetMapping("{city}/{units}")
    public String weather3 (@PathVariable(name="city", required = true) String city, @PathVariable(name="units", required = true) String units, Model model) {
        model.addAttribute("city", city);
        model.addAttribute("units", units);
